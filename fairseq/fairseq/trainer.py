...
class Trainer(object):
    ...
    def _build_optimizer(self):
        if self.pruning_lr is None:
            params = [
                {'params': [p for n, p in self.model.named_parameters() if n.split('.')[-1] != "log_a" and p.requires_grad]},
            ...
            print('log_a lr is %.4f' % params[2]['lr'])
    ...
    def lr_step(self, epoch, val_loss=None):
        ...
        # prefer updating the LR based on the number of steps
        return self.lr_step_update(val_loss)

    def lr_step_update(self, val_loss=None):
        """Update the learning rate after each update."""
        if str(self.cfg.lr_scheduler['_name']) == 'tri_stage':
            new_lr = self.lr_scheduler.step_update(self.get_num_updates(), val_loss)
        else:
