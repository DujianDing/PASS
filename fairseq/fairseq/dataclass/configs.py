...
class PruningConfig(FairseqDataclass):
    ...
    # PASS Arguments
    sparsity_rate: Optional[float] = field(
        default=None,
        metadata={"help": "user specified sparsity rate"},
    )

    reg_coeff: Optional[float] = field(
        default=None,
        metadata={"help": "regularization coefficient"},
    )
...
class CommonEvalConfig(FairseqDataclass):
    ...
    head_skip: bool = field(default=False, metadata={"help": "skip pruned attention layers"})
    head_select: bool = field(default=False, metadata={"help": "only compute unpruned attention heads (in unskipped layers)"})
    track_energy: bool = field(default=False, metadata={"help": "track model-wise energy usage"})
    track_latency: bool = field(default=False, metadata={"help": "track attn-layer-wise latency"})
    approach_name: str = field(default='PASS', metadata={"help": "name of the approach under test"})
