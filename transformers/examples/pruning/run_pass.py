...
class PruningArguments:
    ...
    joint_pruning: Optional[bool] = field(
        default=False,
        metadata={"help": "if set, train gate variables and other parameters in the original model together."}
    )
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        model_args, pruning_args, data_args, training_args = parser.parse_json_file(
    ...
    target_sparsity = 1 - pruning_args.num_of_heads / 144
    print('target sparsity is', target_sparsity)
    ...
    print('pruning masks')
    print(head_mask)
    logger.info("l0 penalty: {}, remaining heads: {}, accuracy: {}".format(pruning_args.l0_penalty, head_mask.sum(),
                                                                           score * 100))
